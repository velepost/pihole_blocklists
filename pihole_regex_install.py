...
url_regexps_remote = 'https://www.myvelecloud.com/cgi-bin/hello.pl?action=regex'
...
if os.path.exists(path_legacy_regex):
    print('[i] regex.list path exists')
else:
    print(f'[e] {path_legacy_regex} was not found')
	
if os.path.exists(path_legacy_mmotti_regex):
    print('[i] mmotti-regex.list path exists')
else:
    print(f'[e] {path_legacy_mmotti_regex} was not found')
